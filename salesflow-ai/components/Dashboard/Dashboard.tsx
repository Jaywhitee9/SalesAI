...
import { DASHBOARD_KPIS, TODAYS_CALLS, DASHBOARD_TASKS, COACHING_TIPS } from '../../constants';
...
export const Dashboard: React.FC<DashboardProps> = ({ onStartCall, isDarkMode }) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 lg:p-10 bg-transparent" dir="rtl">
                   ...
                   <tr key={call.id} className="group hover:bg-brand-50/30 dark:hover:bg-brand-900/10 transition-colors">
                     ...
                     <td className="px-6 py-4 align-middle">
                       {call.status === 'In Progress' ? (
                          ...
                          <button className="h-8 px-3 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-xs hover:bg-white dark:hover:bg-slate-800 hover:border-brand-300 hover:text-brand-600 transition-all flex items-center gap-1.5 bg-slate-50 dark:bg-slate-900">
                             <Phone className="w-3 h-3" /> התחל
                          </button>
                       ) : (
                          <button className="text-xs text-slate-400 hover:text-brand-600 underline decoration-slate-300 hover:decoration-brand-300 transition-all">פרטים</button>
                       )}
                     </td>
                   </tr>
                 ))}
