          ...
          {stages.map((stage, index) => {
            ...
            return (
              <div key={stage.id} className="relative flex gap-4 pb-8 group min-h-[3rem]">
                {/* Connecting Line */}
                {!isLast && (
                  <div className={`absolute right-[9px] top-6 bottom-0 w-px ${isCompleted ? 'bg-brand-200 dark:bg-brand-800' : 'bg-slate-100 dark:bg-slate-800'}`} />
                )}

                {/* Icon/Dot */}
                <div className={`
                  relative z-10 w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 transition-all mt-0.5
                  ${isCompleted ? 'bg-brand-100 text-brand-600 dark:bg-brand-900/50 dark:text-brand-400' : ''}
                  ${isCurrent ? 'bg-brand-600 text-white shadow-md shadow-brand-500/30 ring-2 ring-brand-100 dark:ring-brand-900/30' : ''}
                  ${stage.status === 'upcoming' ? 'bg-white border border-slate-200 text-slate-300 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-600' : ''}
                `}>
                  {isCompleted ? <Check className="w-3 h-3" /> : <div className={`w-1.5 h-1.5 rounded-full ${isCurrent ? 'bg-white' : 'bg-current'}`} />}
                </div>

                {/* Content */}
                <div className={`flex flex-col pt-0.5 transition-opacity ${stage.status === 'upcoming' ? 'opacity-50' : 'opacity-100'}`}>
                  <span className={`text-sm font-medium leading-none ${isCurrent ? 'text-brand-700 dark:text-brand-300 font-bold mb-1.5' : 'text-slate-700 dark:text-slate-300'}`}>
                    {stage.label}
                  </span>

                  {isCurrent && (
                    <div className="mt-1 bg-brand-50 dark:bg-brand-900/20 p-2 rounded-lg border border-brand-100 dark:border-brand-800/50">
                      <p className="text-xs text-brand-800 dark:text-brand-200 leading-snug">
                        {stage.description}
                      </p>
                    </div>
                  )}
