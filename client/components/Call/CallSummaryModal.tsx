...
// Make sure to export the interface so App.tsx and Context can use it if needed
// effectively replacing the old one
export interface CallSummaryData {
    score: number;
    summary: string;
    // We might keep other fields from AI backend even if not displayed yet
    key_points?: { positive: string[], improvements: string[] };
    is_success?: boolean;
}
...
interface CallSummaryModalProps {
    ...
    leadName: string;
    aiSummary: CallSummaryData | null;
}

export const CallSummaryModal: React.FC<CallSummaryModalProps> = ({ isOpen, onClose, onSave, leadName, aiSummary }) => {
    const [outcome, setOutcome] = useState<string | null>(null);
    const [summaryText, setSummaryText] = useState("");
    const [nextStepDate, setNextStepDate] = useState("מחר, 10:00");

    // Pre-fill summary from AI when it arrives or modal opens
    useEffect(() => {
        if (aiSummary?.summary) {
            setSummaryText(aiSummary.summary);
        }
    }, [aiSummary]);
            ...
            <div className="relative w-full max-w-lg bg-white dark:bg-slate-950 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300 border border-slate-200 dark:border-slate-800">
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">עם {leadName} • {new Date().toLocaleDateString()}</p>
                    </div>
                    ...
                    <div className="flex flex-col items-end">
                        ...
                        <div className="flex items-center gap-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-3 py-1 rounded-full shadow-sm">
                            ...
                            <span className="font-bold text-slate-900 dark:text-white">{aiSummary?.score || 0}</span>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {outcomes.map((item) => (
                                <button
                                    ...
                                    className={`flex items-center gap-3 p-3 rounded-xl border-2 transition-all text-right ${outcome === item.id
                    ...
                    <div className="relative group">
                        ...
                        <div className="relative bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800">
                            ...
                            <textarea
                                ...
                                className="w-full text-sm text-slate-700 dark:text-slate-300 bg-transparent border-none p-0 focus:ring-0 resize-none leading-relaxed h-20 placeholder:text-slate-300"
                                placeholder="ממתין לסיכום מה-AI..."
                            />
                        </div>
                    </div>
                ...
                <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex justify-between items-center">
                    ...
                    <Button onClick={() => onSave({ outcome, summary: summaryText })} disabled={!outcome} className="px-6 shadow-lg shadow-brand-500/20">
